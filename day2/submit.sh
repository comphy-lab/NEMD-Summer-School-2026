...
#
#   Run it FROM INSIDE a measurement folder (local_viscosity/ liquid_density/
#   conductance/) so the job starts where `include ../shared_setup.lmp` resolves:
#
#     cd local_viscosity     && ../submit.sh local_viscosity.in
#     cd liquid_density        && ../submit.sh liquid_density.in -var localstress 1 -var dump 1
#
...
if [ ! -f "$IN" ]; then
  ...
  echo "       (local_viscosity/ liquid_vapour/) and run ../submit.sh from inside it."
  exit 1
fi
